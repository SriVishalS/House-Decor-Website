...
jQuery(document).ready(function () {
  $.getJSON(
    "http://api.flickr.com/services/feeds/photos_public.gne?id=90412460@N00&lang=en-us&format=json&jsoncallback=?",
    function (data) {
      $.each(data.items, function (i, item) {
        if (i <= 8) {
          // <— change this number to display more or less images
          $("<img/>")
            .attr("src", item.media.m.replace("_m", "_s"))
            .appendTo(".FlickrImages ul")
            .wrap(
              "<li><a href='" +
                item.link +
                "' target='_blank' title='Flickr'></a></li>"
            );
        }
      });
    }
    ...
    return this.each(function () {
      ...
      var x = function () {
        ...
        if (n.debug) {
          ...
          console.log(
            "Matched element: " +
              (s.attr("class") !== undefined
                ? s.prop("tagName").toLowerCase() + "." + s.attr("class")
